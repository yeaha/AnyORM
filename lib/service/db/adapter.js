...
Adapter.prototype.insert = function(table, data, returning) {
    ...
    return Q.ninvoke(pool, 'acquire')
            .then(function(conn) {
                var promise = Q.ninvoke(conn, 'query', sql, params);
                ...
                if (returning.length)
                    promise.spread(function() {
                        return _.object(returning, _.values(arguments));
                    });
...
_driver['postgres'] = {
    ...
    _lastIdStatement: function(table, column) {
        ...
        return util.format('SELECT CURRENT(%s)', this.quoteIdentifier(sequence));
    },
};
