...
Adapter.prototype.update = function(table, data, where, where_values) {
    ...
    return this.execute(sql, values).then(function(result) { return result.rowCount; });
};
...
_driver['postgres'] = {
    ...
    insert: function(table, data, returning) {
        ...
        sql += ' RETURNING '+ _.map(returning, this.quoteIdentifier.bind(this)).join(', ');
...
_driver['sqlite3'] = {
    ...
    insert: function(table, data, returning) {
        ...
        var self = this;
        var query = this._insertStatement(table, data);
...
ProcessorStream.prototype._transform = function(row, encoding, callback) {
    try {
