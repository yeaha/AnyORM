...
let Type = require(__dirname+'/type');
let Service = require(__dirname+'/service');
...
class Mapper extends EventEmitter {
    constructor(options) {
        ...
        attributes.keys().forEach(key => {
            let attribute = Type.normalizeAttribute(attributes[key]);
    ...
    getService() {
        let service = this.getOption('service');
    ...
    pack(record, data) {
        ...
        record.keys().forEach(key => {
            ...
            if (attribute) {
                ...
                values[key] = Type.get(attribute.type).restore(value, attribute);
            }
        });
    ...
    destroy(data) {
        ...
        return this.doDelete(data).then(() => {
    ...
    // return Promise
    _find(id, data) {
        return this.doFind(id).then(record => {
            if (!record) {
                return false;
            }
    ...
    _insert(data) {
        ...
        return this.doInsert(data).then(primary_values => {
            let keys = [];
            for (let key of this.getPrimaryKey()) {
                if (!data.get(key)) {
                    keys.push(key);
                }
            }
            ...
            let values = {};
            if (_.isPlainObject(primary_values)) {
                ...
                values[keys.shift()] = primary_values;
            }
    ...
    _update(data) {
        ...
        return this.doUpdate(data).then(() => {
            this.pack({}, data);
...
//     var service = this.getOption('service');
//     return Services.get(service, arguments);
// };
//
// /**
...
//
//         if (value !== null) {
//             value = Type.get(attribute.type).store(value, attribute);
//         }
//
//         record[key] = value;
//     }, this);
