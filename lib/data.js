...
Data.prototype.set = function(key, value, options) {
    ...
    // compare cloned value is unnecessary
    if (!attribute.clone && this.get(key) === value) {
        return this;
    }
 ...
 * @param {string} key
 * @param {object} options
 * @param {boolean} [options.clone] return clone of property value, will overwrite attribute "clone"
 *
 * @throws property is undefined
 *
 * @return {?*}
 */
Data.prototype.get = function(key, options) {
    options = options || {};
    ...
    if (value === undefined) {
        return Type.get(attribute.type).getDefaultValue(attribute);
    } else if (!value) {
        return value;
    }
 ...
 * @example
 * data.pick('foo');
 * data.pick('foo', 'bar', {clone: true});
 * data.pick(['foo', 'bar'], {clone: true});
 * data.pick({clone: true});
 *
 * @param {string|array|object} keys
 * @param {object} options
 * @param {boolean} [options.clone] Same as data.get() options
 * @return {object}
 */
Data.prototype.pick = function(keys, options) {
    function _filter(keys) {
        return _.intersection(_.keys(attributes), keys);
    }
    ...
    if (_.isString(keys)) {
        ...
        keys = _filter(keys);
    } else if (keys === undefined || _.isObject(keys)) {
        options = keys;

        keys = [];
        _.each(attributes, function(attr, key) {
            if (!attr.protected && _.has(this._values, key)) {
                keys.push(key);
            }
        }, this);
    }
