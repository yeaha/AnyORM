...
var DBMapper = exports.DBMapper = Mapper.define({
    ...
    doSearch: function(expression, options) {
        var adapter = this.getService();
        var select = adapter.select(this.getCollection());

        _.each(expression, function(value, key) {
            var column = adapter.quoteIdentifier(key);
            select.where(column+' = ?', value);
        });

        return select.get(options);
    },
