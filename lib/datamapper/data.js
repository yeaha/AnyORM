...
function Data(data, is_fresh) {
    ...
    _.each(this.getMapper().getColumn(), function(config, prop) {
        this.__defineGetter__(prop, _.bind(this.get, this, [prop, config]));
...
Data.prototype.get = function(prop, config) {
    if (!(config || this.getMapper().getColumn(prop)))
...
Data.prototype._set = function(prop, value, strict) {
    ...
    value = this._normalizeProp(prop, value, column);
...
Data.prototype._normalizeProp = function(prop, value, config) {
    // if (prop == 'foo')
